...
- (IBAction)add:(id)sender{
	UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:@"Add" message:@"Add to portfolio?" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:nil] autorelease];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
		...
		[symbols addObject: etfSymbolLbl.text];
		vs.symbols = symbols;
		vs.isPortfolioChanged=YES;
		NSLog(@"%@ added to portfolio", etfSymbolLbl.text);
		[self dismissModalViewControllerAnimated:YES];
	}
}
...
//	UITableViewCell *cell = [tv dequeueReusableCellWithIdentifier:@"TradeDetailsCell"];
//	if(nil == cell) {
//		cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:@"TradeDetailsCell"] autorelease];
//	}
...
{
	ETF *etf = self.listTab.selectedETF;
